...
def heuristic(current_map: np.ndarray, current_pos: np.ndarray, goal_pos: np.ndarray):
    """return the Manhattan distance from current pos to goal pos

    Args:
        current_map (np.ndarray): A 120*120 array indicating current map, where 0 indicating traversable and 1 indicating obstacles.
    ...
    """
    res = (goal_pos - current_pos).abs().sum()
    return res
